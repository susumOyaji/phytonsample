...
今回は大手IT企業GAFAの過去の株価データより株価分析を行います。
知らない方のためにGAFAとはGoogle、Amazon、Facebook、Appleという大手IT企業の頭文字をとったものです。
早速、GAFAの株価を取得してみましょう٩( ‘ω’ )و
...
次に株価を取得するためpandas_datareaderをimport します。
pandas_datareaderはweb上の様々なソースに簡単にアクセスして、株価や為替レート、人口などのデータを取得できるものです。
import pandas_datareader as web

次に取得したデータをグラフ化させる。matplotlibをインポートします。
import matplotlib.pyplot as plt

次にここからが重要です。ここで株式シンボルを取得します。
その後に、株式情報の取得日（開始日と終了日）を決めます。
そして、取得したデータからグラフを作成し、単純な株式で得れるリターン(利益)を毎日計算してみましょう٩( ‘ω’ )و
...
皆さんが他に表示したい株があった場合。例えば、ソニー株を表示したい場合は「ソニー ティッカーシンボル」と検索すればすぐに確認できます。
ちなみに、ソニーの場合はSNEで、ソフトバンクの場合はSFTBYと記載すれば取得できます。

次に取得日の開始日と終了日を設定しましょう。今回は2014年から本日までの6年間を取得したいと思います。
startdate=’2014-01-01’
終了日は実行時の日にしたいので、日時と文字列を相互に変換するメソッドstrftime関数を使います。
...
このようにGAFAのデータをグラフ化することができたことが確認できます。x軸にはタイトルもy軸にもタイトルが付いていることが確認できます。
また左上にアンカーがあることも確認できます。
ちなみに私は2016年の時期にAMAZONの株を保有しておりましたが、その翌年にかけて急激に下がってしまったので売却してしまいました。
このグラフを確認しますと保有しておけばと思います。(T . T)

はい、今回は大手IT企業GAFAの過去の株価データからグラフを表示してみました。
ティッカーシンボルから表示したい株を特定し、マットプロットライブを使ってグラフ化まで行いました。

次回のpython finance2では実際の株式運用を想定した、他の株価との相関関係の調べ方やリターンの計算方法や共分散行列やボラリティなどから実際的な運用方法の解説まで行っております。
LSTMを用いたAiで実際に株価の予測はpython finance3で行っています。
具体的にはグーグル社の株価の予測と実際株価結果の確認まで行っています。是非興味がある方はご覧ください。
