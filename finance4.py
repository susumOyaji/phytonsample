...
最後まで見てくださいましたら、移動平均の線がpythonで描けるようになり、テクニカル指標を視覚化する方法について解説を行って行きます。また補足説明として投資における金融工学を少しご紹介します。是非最後までご覧ください。
...
map.rcParams['font.family']='serif'
...
#今回はマイクロソフトの株で確認を行います。マイクロソフトのティッカーシンボルがMSFTです。今回は2014年の1月1日から2020年の本日までの株式情報を取得して行きましょう。
...
microsoft_stock = web.DataReader('MSFT',data_source ="yahoo",start = start_date,end = end_date)["Adj Close"]
...
'''
※投資を初心者の方のために、移動平均とは文字通り過去の履歴の平均値を表示しているものです。投資の世界では単純移動平均を（SMA）と呼び、さらに短期SMAが長期SMAを上回る市場をゴーデンクロスと呼びます。一般に投資家はロングポジションつまり（買い）を入れます。反対に長期SMAが短期SMAを上回る市場をデッドクロスと呼びます。投資家はショートポジションつまり（売り）を入れます。

※ここで少しだけ注意喚起をします。他にもダウ理論やグランビル法則、エリオット波動、ピボット手法など、たくさんの手法はありますが、株はそう簡単ではありません。

社会情勢によっても変動します。前回の動画でも少し触れましたが、テクニカル指標だけで稼ぐには統計学と金融工学を勉強する必要があります。そもそも、どのようにして株の値動きが決まっているのか、インターバンクの仕組みはどのようになっているのか、それを把握する必要があります。

よく目にする投資家などの講演会では、そう言った情報は提供していません。なぜなら彼らは投資で稼いでるのではなく、レベニューシェアや講演家として稼いでるからです。ですので皆さんのお金でご飯を食べている人が大半です。

私の知り合いに統計学と金融工学を駆使し、何億稼いだ成功者はいますがその人は一切情報を公開していません。人間はそういう生き物です。

ですので私たちも知識は提供しますが手法を伝授することは致しません。

人間はそうい生き物なのです。逆を言えば提供している人たちは案件であり皆さんのお金をとっているだけです。オンラインサロンなんかも同じことです。現代版の一種の宗教団体が少しビジネスかじっただけです。

結局のところ成功するためには、自信の作品であり、製品であり、アプリなどを作りサービスを提供しないといけません。

②rollingを使って移動平均を計算します。ローリング統計を用いて単純移動平均を表示してみます。まずはマイクロソフトのデータを価格のみにしてあげましょう。次に短期SMAを設定します。(25期間の)
'''
