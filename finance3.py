...
#次は前回と同じように株価を取得していきます。今回は2012年の1月1日から2020年の6月までの株式情報を取得して行きましょう。

sony_stock = web.DataReader("SNE",data_source="yahoo",start="2020-01-01",end="2020-06-01")
...
#ここからがすごく重要です。予測したデータと実際の結果がどれだけ似ているのか検証するためにも機会学習をさせるためデータを分ける必要があります。つまり、実際にデータの80％を使用して残りの20％のデータを予測させ、実際のデータと比較します。ですので訓練用データとテスト用データに分けましょう。訓練用データは、先ほどのデータに0.8かけてtrainig_data変数を用意します。
...
# 青色が機械学習を行った実際の過去のデータです。黄色が機械学習によって予測されたデータです。
# 赤色がその時の実際の株価データです。
# はい、今回はグーグル社の過去の株価データからLSTMを用いて株価分析まで行ってみました。
